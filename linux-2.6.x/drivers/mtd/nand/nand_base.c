...
#if defined(CONFIG_MTD_NAND_BBM) 
/**
 * nand_replaced_block - Read bad block marker from the chip
 * @mtd:	MTD device structure
 * @ofs:	offset from device start
 *
 * return replaced block from reserved pool - offset from device start
 * (it is necessary to recursivly call nand_replaced_block if a bad block
 * returns from reserved pool.)
 */
loff_t nand_replaced_block(struct mtd_info *mtd, loff_t ofs)
{
	struct nand_chip *this = mtd->priv;
	int orig_block;
	loff_t page_ofs, replaced_block_ofs;
	...
	/* check, if not replacement returns original offset back */
	if (this->bbm[orig_block] == 0xffff)
		return ofs;

	/* get the replacement block */
	replaced_block_ofs = (this->bbm[orig_block] << this->phys_erase_shift) + rsvdblock_offset + page_ofs;
	DEBUG (MTD_DEBUG_LEVEL0, "original bad block# %d with offset 0x%08x, replaced_block_ofs 0x%08x\n", 
		orig_block, (unsigned int)ofs, (unsigned int)replaced_block_ofs);

	/* make sure the replacement is a good block */
	if (nand_isbad_bbt(mtd, replaced_block_ofs, 0)) {
		/* recursive for replacement if necessary */ 
		replaced_block_ofs = nand_replaced_block(mtd, replaced_block_ofs);
	}

	/* must have the good block for nthe replacement */
	return replaced_block_ofs;
}
 ...
 */
	if (nand_isbad_bbt (mtd, ofs, allowbbt)) {  /* bbt said: bad block */
		if (ofs != nand_replaced_block(mtd, ofs))
...
retry:
	 ...
	 */
	if (this->owner == current && this->state == FL_RDDIST_FIXING) {
		spin_unlock(lock);
		return 0;
	}
#endif
...
#endif
{
	...
	if (nand_isbad_bbt (mtd, from, 0)) {
		from_rb = nand_replaced_block(mtd, from);
		if (from_rb != from) {
		...
		*/ 
		...
		if (!(page & blockcheck)) {
			...
			if (nand_isbad_bbt (mtd, (realpage << this->page_shift), 0)) {
				from_rb = nand_replaced_block(mtd, (realpage << this->page_shift));
...
static int nand_read_distfix (struct mtd_info *mtd, int rd_dist_block, int reason_code) 
{
	...
	size_t rlen;
	int i, ret, status, numpages, rsvdblock_idx, chipnr, rdbackblock=0, numoobfree=0;
	...
	if (nand_isbad_bbt (mtd, from, 0))
		from = nand_replaced_block(mtd, from);
	...
	DEBUG(MTD_DEBUG_LEVEL1,"blockoffset:%08x, read_dist_block:%d, " 
			"rsvdblock_offset:%08x, rsvdblock_idx:%d\n", 
	...
	if (ret < 0) {
		printk (KERN_ERR "step 3:nand_do_read_ecc failed on offset: 0x%08x\n", (unsigned long)from);
		goto done;
	}
	...
	nand_release_device(mtd);
	if (status & NAND_STATUS_FAIL) {
		printk (KERN_ERR "step 4.1:nand_erase_cmd failed on offset: 0x%08x\n", (unsigned long)to);
	...
	if (ret < 0) {
		printk (KERN_ERR "step 4.2:nand_write_ecc failed on offset: 0x%08x\n", (unsigned long)to);
		/* mark this reserved block as bad blopck */
		nand_do_block_mark(mtd, to, 0x01);
		goto done;
	}
	...
	if (memcmp(mainbuf, rdbackbuf, mtd->erasesize) != 0) {
		printk (KERN_ERR "step 4.3:block %d - data read back check failed on offset: 0x%08x\n",
	...
	if (memcmp(oobbuf, rdbackoob, numoobfree*numpages) != 0) {
		printk (KERN_ERR "step 4.3:block %d  - oob read back check failed on offset: 0x%08x\n",
			rd_dist_block, (unsigned long)to);
		/* mark this reserved block as bad blopck */
		nand_do_block_mark(mtd, to, 0x01);
		goto done;
	...
	ret = update_bbm(mtd, this->bbm, to>>this->chip_shift);
	if (ret < 0) {
	...
	nand_release_device(mtd);
	if (status & NAND_STATUS_FAIL) {
		printk (KERN_ERR "step 8:nand_erase_cmd failed on offset: 0x%08x\n", (unsigned long)to);
	...
	if (memcmp(mainbuf, rdbackbuf, mtd->erasesize) != 0) {
		printk (KERN_ERR "step 10:block %d  - data read back check failed on offset: 0x%08x\n",
			rd_dist_block, (unsigned long)to);
		/* make the read disturb error block as permanetal bad block, replaced with
		 * replaced with a good block from reserved pool */
		goto done;
	...
	if (memcmp(oobbuf, rdbackoob, numoobfree*numpages) != 0) {
		printk (KERN_ERR "step 10:block %d  - oob read back check failed on offset: 0x%08x\n",
			rd_dist_block, (unsigned long)to);
		/* make the read disturb error block as permanetal bad block, replaced with
		 * replaced with a good block from reserved pool */
		goto done;
	...
	ret = update_bbm(mtd, this->bbm, from>>this->chip_shift);
	if (ret < 0) {
			   ...
			   size_t * retlen, const u_char * buf, u_char * eccbuf, struct nand_oobinfo *oobsel)
{
	...
	*retlen = 0;
	...
	if (nand_isbad_bbt (mtd, to, 0)) {
		to_rb = nand_replaced_block(mtd, to);
		if (to_rb != to ) {
		...
		*/
		if (!(page & (ppblock - 1))){
			...
			*retlen = written;
			...
			if (nand_isbad_bbt (mtd, (page << this->page_shift), 0)) {
				to_rb = nand_replaced_block(mtd, (page << this->page_shift));
...
int nand_erase_nand (struct mtd_info *mtd, struct erase_info *instr, int allowbbt)
{
	...
	while (len) {
		...
		if (nand_isbad_bbt (mtd, (page << this->page_shift), allowbbt)) {
			instr->addr = nand_replaced_block(mtd, (page << this->page_shift));
...
int nand_scan (struct mtd_info *mtd, int maxchips)
{
	...
	*/
	...
	if (!(g_write_test_info.blk_tbl = kmalloc(g_read_dist_info.numblocks, GFP_KERNEL))) {
		printk (KERN_ERR "g_write_test_info.blk_tbl - Out of memory (size %d)\n", g_read_dist_info.numblocks);
		return -ENOMEM;
	}
	memset(g_write_test_info.blk_tbl, 0x0, g_read_dist_info.numblocks);
	/* allocate memory for g_erase_test_info.blk_tbl - (1 byte per block) */
	if (!(g_erase_test_info.blk_tbl = kmalloc(g_read_dist_info.numblocks, GFP_KERNEL))) {
		printk (KERN_ERR "g_erase_test_info.blk_tbl - Out of memory (size %d)\n", g_read_dist_info.numblocks);
		return -ENOMEM;
	}
	memset(g_erase_test_info.blk_tbl, 0x0, g_read_dist_info.numblocks);
#endif /*CONFIG_MOT_FEAT_NAND_BLKCNT_TEST*/
#endif /*CONFIG_MOT_FEAT_NAND_RDDIST*/
