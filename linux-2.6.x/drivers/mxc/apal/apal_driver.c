...
#define APAL_SDMA_TIMEOUT  (HZ)  // HZ - 1sec, SDMA timeout - 1sec
...
)
{
    ...
    /* Turn on this when debugging bd_done bits */   
#if 0
...
)
{
    ...
    /* enable clock if in master mode only */
    ...
    /* enable clock if in master mode */
...
)
{
	...
	case APAL_GET_LEFT_WRITTEN_BYTES_IN_KERNEL:
	    unplayed_buffer_number = (apal_tx_dma_config[stdac_codec_in_use].next_buffer_to_fill + 
            APAL_NB_BLOCK_SDMA - apal_tx_dma_config[stdac_codec_in_use].buffer_read)%APAL_NB_BLOCK_SDMA;
