    ...
    case RTC_SW_SETTIME_FUZZ:
        ...
        *_reg_EPIT_EPITCR &= ~EPITCR_EN;
        ...
        if (fuzz != NULL) {
            ...
            req->remain = hundredths_to_ticks(hundredths);
        }
...
static unsigned long saved_gptcnt;
...
static int rtcsw_suspend(struct device *dev, u32 state, u32 level)
{
	if (level == SUSPEND_POWER_DOWN) {
		...
		saved_gptcnt = __raw_readl(MXC_GPT_GPTCNT);
		correct_time = 1;
	}
...
static int rtcsw_resume(struct device *dev, u32 level)
{
	struct timespec tv;
        ...
        unsigned long curtime, cur_gptcnt, diff_gptcnt_ns;
	unsigned long flags;
	...
	case RESUME_POWER_ON:
		...
		if (!correct_time) {
			...
			correct_time = 0;
                ...
                /* measure the over-compensation */
                cur_gptcnt = __raw_readl(MXC_GPT_GPTCNT);
                cur_gptcnt -= saved_gptcnt;

                /* convert into ns */
                diff_gptcnt_ns = (cur_gptcnt*1000)/(CLOCK_TICK_RATE/1000000);
