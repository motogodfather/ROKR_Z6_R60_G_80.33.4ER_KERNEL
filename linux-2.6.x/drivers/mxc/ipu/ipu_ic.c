...
/*     R = (1.164 * (Y - 16)) + (1.596 * (Cr - 128));
	...
	if ((in_format == YCbCr) && (out_format == RGB)) {
		...
		DPRINTK("addr 0x%04X: word0 = 0x%08X, word1 = 0x%08X\n",
		...
		DPRINTK("TODO: Unsupported color space conversion\n");
	}
				...
				uint32_t * downsizeCoeff)
{
	...
	*resizeCoeff = (8192L * (tempSize - 1)) / (outSize - 1);
	if (*resizeCoeff >= 16384L) {
		DPRINTK("Warning! Overflow on resize coeff.\n");
		*resizeCoeff = 0x3FFF;
	}

	DPRINTK("resizing from %d -> %d pixels, "
