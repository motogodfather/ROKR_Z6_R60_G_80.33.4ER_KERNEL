...
static uint32_t hsync_to_data;
static uint32_t vsync_to_data;
			   ...
			   ipu_di_signal_cfg_t sig)
{
	...
	reg =
	    ...
	    ((uint32_t) (width + hSyncWidth + hStartWidth + hEndWidth - 1) << 16);
	__raw_writel(reg, SDC_HOR_CONF);

	reg = ((uint32_t) (vSyncWidth - 1) << 26) | SDC_V_SYNC_WIDTH_L |
	    ((uint32_t) (height + vSyncWidth + vStartWidth + vEndWidth - 1) << 16);
	__raw_writel(reg, SDC_VER_CONF);

	hsync_to_data = hSyncWidth + hEndWidth;
	vsync_to_data = vSyncWidth + vEndWidth;

	switch (panel) {
	case IPU_PANEL_SANYO_TFT:
#ifdef CONFIG_MOT_FEAT_POWERUP_LOGO
                __raw_writel(SDC_COM_TFT_COLOR | SDC_COM_FG_EN | SDC_COM_BG_EN, SDC_COM_CONF);
#else
...
int32_t ipu_sdc_set_window_pos(ipu_channel_t channel, int16_t x_pos, int16_t y_pos)
{
	FUNC_START;
