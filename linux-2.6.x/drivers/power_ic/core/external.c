...
/*!
 * @brief Masks indicating which bits are maintained in the RAM copy of each register when reading
 *
 * A '1' bit indicates that the corresponding bit in the power IC register will not be saved in the
 * RAM copy of the register during a read, and the cached value of the register will be used during
 * a masked write.
 *
 ...
 */
static const unsigned int power_ic_register_no_write_when_read_masks[POWER_IC_REG_NUM_REGS] =
{
    /* Atlas Registers */
    0x0000000,  /* Interrupt Status 0 */
    0x0000000,  /* Interrupt Mask 0 */
...
static int read_reg(POWER_IC_REGISTER_T reg, unsigned int *reg_value) 
{
    ...
    if (retval == 0)
    {
        ...
        power_ic_registers[reg] |= value & ~power_ic_register_no_write_when_read_masks[reg];
    }
...
int power_ic_set_reg_mask(POWER_IC_REGISTER_T reg, int mask, int value)
{
    ...
    if ((read_before_write[reg] & READ_BEFORE_WRITE) && (power_ic_register_no_write_when_read_masks[reg] == 0x0000000))
    {
        /*
    ...
    /* Bits 15 and 16 in Power Miscellaneous need to be inverted before writing */
    if(reg == POWER_IC_REG_ATLAS_PWR_MISC)
    {
        old_value ^= POWER_GATE_INVERT;
    }
