 ...
 * Motorola 2006-May-08 - Add permissions to RTC setting
...
/*==================================================================================================
    ...
    Module used to read the RTC registers.
...
int rtc_ioctl(unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case POWER_IC_IOCTL_SET_TIME:
            ...
            /* Verify the application trying to set the time of the RTC has permission to do so */
            if (! capable(CAP_SECURE_CLOCK))
            {
                err = -EPERM; 
                break; 
            } 
...
#ifdef CONFIG_MOT_FEAT_SECURE_CLOCK 
            /* Verify the application trying to set the alarm time of the RTC has permission to do so */
            if (! capable(CAP_SECURE_CLOCK))
            {
                err = -EPERM; 
                break; 
